    ...
    def __init__(self, world, ego_vehicles, config, randomize=False,
        ...
        if self._randomize is True:
            ...
            if (scenario_config == -1):
                ...
                if((scenario_config >> 1) % 2): #second last Bit sets if Person crosses road 
                    self._cross_lane = True
                else:
                    self._cross_lane = False
                if((scenario_config >> 2) % 2): #third last Bit sets the blocker
                    self._spawn_blocker_bool = True
                else:
                    self._spawn_blocker_bool = False
                if((scenario_config >> 3) % 2): #forth last Bit spawns the container on the road
